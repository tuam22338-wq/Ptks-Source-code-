...
const CustomStoryPlayer: React.FC<CustomStoryPlayerProps> = ({ gameState, setGameState }) => {
    ...
    if (!activeStory || !storySystem || !currentNode) {
        // Fallback: End story if data is missing
        if (activeStory) {
    ...
    // This is a simplified outcome processor for stories. A more complex one would be needed for all event types.
    const applyOutcomes = (outcomes: any[]) => {
        if (!outcomes || outcomes.length === 0) return;

        outcomes.forEach(outcome => {
             if (outcome.type === 'CHANGE_STAT') {
                 // Simplified: only handles stat changes
                 setGameState(gs => {
                    if (!gs) return null;
