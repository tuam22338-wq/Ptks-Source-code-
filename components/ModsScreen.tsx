    ...
    return (
        <div className="bg-black/20 p-4 rounded-lg border border-gray-700/60 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fade-in" style={{animationDuration: '300ms'}}>
            <div className="flex-grow">
                <h3 className="text-lg font-bold font-title text-gray-200">{mod.modInfo.name}</h3>
                {mod.modInfo.author && <p className="text-sm text-gray-400">bởi {mod.modInfo.author}</p>}
            ...
            <main className="flex-grow min-h-[400px]">
                {activeTab === 'library' && (
                    <div className="space-y-4 animate-fade-in" style={{animationDuration: '300ms'}}>
                        <h3 className="text-xl font-bold font-title text-gray-300">Thư Viện Mod</h3>
                        <p className="text-gray-400">Các gói nội dung tạo sẵn để làm phong phú thêm trải nghiệm của bạn.</p>
                ...
                {activeTab === 'import' && (
                    <div className="space-y-4 animate-fade-in" style={{animationDuration: '300ms'}}>
                        <div className="flex justify-between items-center">
                            <h3 className="text-xl font-bold font-title text-gray-300">Mods Đã Cài Đặt</h3>
                ...
                {activeTab === 'create' && (
                    <div className="text-center text-gray-400 p-8 bg-black/20 rounded-lg border border-dashed border-gray-700 h-full flex flex-col justify-center items-center animate-fade-in" style={{animationDuration: '300ms'}}>
                        <h3 className="text-2xl font-bold font-title text-gray-200">Trở thành người sáng tạo</h3>
                        <p className="mt-2 mb-6 max-w-2xl mx-auto">Sử dụng trình chỉnh sửa mạnh mẽ với sự hỗ trợ của GameMaster AI để tạo ra các vật phẩm, nhân vật, và thậm chí cả hệ thống tu luyện của riêng bạn.</p>
                        <button onClick={() => onNavigate('createMod')} className="flex items-center justify-center gap-3 w-64 h-16 mx-auto bg-teal-700/80 text-white text-xl font-bold font-title rounded-md border-2 border-teal-600 transition-all duration-300 ease-in-out transform hover:scale-105 hover:bg-teal-600/80 shadow-lg shadow-black/30">
