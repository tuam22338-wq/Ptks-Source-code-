...
const GuideSection: React.FC<{ icon: React.ElementType; title: string; children: React.ReactNode }> = ({ icon: Icon, title, children }) => (
    <div className="neumorphic-inset-box p-3">
        <h4 className="flex items-center gap-2 font-bold font-title" style={{color: 'var(--primary-accent-color)'}}>
            <Icon className="w-5 h-5" />
            {title}
        </h4>
        <div className="mt-2 text-sm space-y-2 pl-4 border-l-2 ml-2" style={{color: 'var(--text-color)', borderColor: 'var(--shadow-light)'}}>
            {children}
        </div>
    </div>
);
...
const GuidePanel: React.FC = () => {
    return (
        <div className="space-y-4 animate-fade-in" style={{ animationDuration: '300ms' }}>
            <h3 className="flex items-center justify-center gap-2 text-lg font-title font-semibold mb-4 text-center border-b pb-2" style={{color: 'var(--text-color)', borderColor: 'var(--shadow-light)'}}>
                <FaBookReader style={{color: 'var(--primary-accent-color)'}} /> Hướng Dẫn Nhanh
            </h3>

            <GuideSection icon={GiGears} title="Sử Dụng Giao Diện">
                <p>Các bảng thông tin và chức năng chính có thể được truy cập như sau:</p>
                <ul className="list-disc list-inside italic" style={{color: 'var(--text-muted-color)'}}>
                    <li><strong style={{color: 'var(--text-color)'}}>Bảng Điều Khiển:</strong> Mở bảng điều khiển chính (Trạng Thái, Bản Đồ, Nhiệm Vụ, v.v.) qua nút trên thanh hành động.</li>
                    <li><strong style={{color: 'var(--text-color)'}}>Túi Đồ (<GiSwapBag className="inline-block mb-1"/>):</strong> Gõ lệnh "mở túi đồ" hoặc nhấn nút nhanh để mở túi đồ của bạn.</li>
                    <li><strong style={{color: 'var(--text-color)'}}>Hỏi Thiên Cơ:</strong> Sử dụng tab "Hỏi Thiên Cơ" trong ô hành động để hỏi AI về các thông tin trong game.</li>
                </ul>
            </GuideSection>
            ...
            <GuideSection icon={GiCastle} title="Phát Triển Sức Mạnh">
                ...
                <p>Tuy nhiên, để thực sự mạnh mẽ, bạn cần tìm và đi theo một <strong style={{color: 'var(--primary-accent-color)'}}>con đường sức mạnh chính</strong> (ví dụ: công pháp chủ đạo, cây kỹ năng, cấy ghép cybernetic...). Con đường này sẽ mở khóa các khả năng độc đáo và định hình phong cách chiến đấu của bạn.</p>
                <ul className="list-disc list-inside italic mt-2" style={{color: 'var(--text-muted-color)'}}>
             ...
             <GuideSection icon={GiGears} title="Các Hành Động Hệ Thống">
                ...
                <ul className="list-disc list-inside italic" style={{color: 'var(--text-muted-color)'}}>
                    <li><strong style={{color: 'var(--text-color)'}}>Gia nhập Phe phái (<FaLandmark className="inline-block mb-1"/>):</strong> "tìm một phe phái để gia nhập", "xin gia nhập Vệ Binh Tinh Hệ".</li>
                    <li><strong style={{color: 'var(--text-color)'}}>Căn cứ (<GiMountainCave className="inline-block mb-1"/>):</strong> "nâng cấp phòng thí nghiệm", "cải thiện khu vườn", "xây dựng Tụ Linh Trận".</li>
                    <li><strong style={{color: 'var(--text-color)'}}>Chế tạo (<GiCauldron className="inline-block mb-1"/>):</strong> "bắt đầu chế tạo", "tạo ra thuốc hồi phục", "luyện chế Hồi Khí Đan". AI sẽ tự kiểm tra xem bạn có bản thiết kế/đan phương, nguyên liệu, và công cụ hay không.</li>
                </ul>
            </GuideSection>
