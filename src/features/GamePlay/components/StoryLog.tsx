...
import type { StoryEntry, NPC, GameState } from '../../../types';
...
const NpcTooltip: React.FC<{ npc: NPC; gameState: GameState; onClose: () => void; position: { top: number; left: number } }> = ({ npc, gameState, onClose, position }) => {
    ...
    const renderAttributeGroup = (group: any) => {
        ...
        return (
            <div key={group.id} className="bg-black/20 p-2 rounded-md">
    ...
    return (
        <div
            className="fixed z-50 w-80 max-w-[80vw] bg-stone-900/90 backdrop-blur-md border-2 border-amber-500/50 rounded-lg shadow-2xl shadow-black/50 text-white animate-fade-in flex flex-col max-h-[70vh]"
            style={{ top: position.top, left: position.left, animationDuration: '200ms' }}
            onClick={e => e.stopPropagation()}
        >
            <div className="p-3 border-b border-amber-500/30 flex justify-between items-center">
                    ...
                    case 'system-notification':
                        return (
                             <div key={entry.id} className="group relative my-4 p-3 bg-blue-900/20 border-l-4 border-blue-400 rounded-r-lg animate-fade-in" style={animationStyle}>
                                <p className="font-mono text-blue-300 whitespace-pre-wrap">{renderContent()}</p>
