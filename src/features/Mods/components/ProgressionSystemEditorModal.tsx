...
interface ProgressionSystemEditorModalProps {
    isOpen: boolean;
    onClose: () => void;
...
const TierBonusEditor: React.FC<{
    ...
    return (
        <div className="mt-3 pt-3 border-t" style={{borderColor: 'var(--shadow-light)'}}>
            <h5 className="text-sm font-semibold mb-2" style={{color: 'var(--text-color)'}}>Thuộc Tính Cộng Thêm (Khi Đạt Cấp Bậc)</h5>
...
const ProgressionSystemEditorModal: React.FC<ProgressionSystemEditorModalProps> = ({ isOpen, onClose, onSave, initialSystems, attributeSystem }) => {
    const [tiers, setTiers] = useState<ProgressionTierConfig[]>([]);
    ...
    const [editingSubTier, setEditingSubTier] = useState<{ subTier: ProgressionSubTier | null; tierIndex: number }>({ subTier: null, tierIndex: -1 });

    useEffect(() => {
        if (isOpen) {
            const mainSystem = initialSystems.length > 0 ? initialSystems[0] : null;
            setTiers(mainSystem ? JSON.parse(JSON.stringify(mainSystem.tiers)) : []);
            setSystemInfo({
            ...
            const initialExpanded = (mainSystem?.tiers || []).reduce((acc, tier, index) => {
                acc[tier.id || index] = index === 0;
                return acc;
            }, {} as Record<string, boolean>);
            setExpandedTiers(initialExpanded);
        }
    }, [isOpen, initialSystems]);
    ...
    const handleSaveAndClose = () => {
        onSave([{ ...systemInfo, id: 'main_system', tiers: tiers }]);
        onClose();
    };

    const handleTierChange = (index: number, field: keyof ProgressionTierConfig, value: any) => {
    ...
    const handleAddTier = () => {
        const newTier: ProgressionTierConfig = { id: `tier_${Date.now()}`, name: 'Cấp Bậc Mới', description: '', subTiers: [], bonuses: [] };
    ...
    const handleOpenSubTierModal = (subTier: ProgressionSubTier | null, tierIndex: number) => {
        setEditingSubTier({ subTier, tierIndex });
        setIsSubTierModalOpen(true);
    };
    
    const handleSaveSubTier = (subTier: ProgressionSubTier) => {
        const newTiers = [...tiers];
        const tier = newTiers[editingSubTier.tierIndex];
        const subTierIndex = tier.subTiers.findIndex(s => s.id === subTier.id);
        
        if (subTierIndex > -1) {
            tier.subTiers[subTierIndex] = subTier;
        } else {
            tier.subTiers.push(subTier);
        }
    ...
    const handleDeleteSubTier = (tierIndex: number, subTierIndex: number) => {
        if (window.confirm("Bạn có chắc muốn xóa cấp bậc phụ này?")) {
            const newTiers = [...tiers];
            newTiers[tierIndex].subTiers.splice(subTierIndex, 1);
            setTiers(newTiers);
        }
    };
                            ...
                            <div key={tier.id || tierIndex} className="rounded-lg" style={{boxShadow: 'var(--shadow-raised)'}}>
                                ...
                                {expandedTiers[tier.id || tierIndex] && (
                                    <div className="p-3 border-t space-y-3" style={{borderColor: 'var(--shadow-light)'}}>
                                        <input value={tier.name} onChange={e => handleTierChange(tierIndex, 'name', e.target.value)} className="input-neumorphic w-full mb-2" placeholder="Tên Cấp Bậc Chính"/>
                                        ...
                                        <div className="mt-4 pt-3 border-t" style={{borderColor: 'var(--shadow-light)'}}>
                                            <h5 className="text-sm font-semibold mb-2" style={{color: 'var(--text-color)'}}>Các Cấp Bậc Phụ</h5>
                                            <div className="space-y-2">
                                                {tier.subTiers.map((subTier, subTierIndex) => (
                                                    <div key={subTier.id || subTierIndex} className="flex justify-between items-center p-2 rounded" style={{boxShadow: 'var(--shadow-pressed)'}}>
                                                        <div>
                                                            ...
                                                            <p className="text-xs" style={{color: 'var(--text-muted-color)'}}>{systemInfo.resourceName}: {!isFinite(subTier.resourceRequired) ? 'Vô Hạn' : (subTier.resourceRequired || 0).toLocaleString()}</p>
